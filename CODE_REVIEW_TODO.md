...
- [ ] **M14 — A CustomExecution-backed task's problem-matcher diagnostics never clear on rerun.**
  ...
  - **Investigated 2026-08-28, could not reproduce -- likely misdiagnosed:** built the suggested fix (moved every contributed task off `vscode.CustomExecution` onto `vscode.ProcessExecution`, spawning a new standalone `taskRunner.ts` helper via `process.execPath` + `ELECTRON_RUN_AS_NODE=1`, argv-only). It built cleanly and all existing tests passed, but repeated attempts (fake compiler scripts in both matcher formats, and the real `pbcompiler` with a genuine syntax error) could not reproduce the original defect on the *old* `CustomExecution` code either. VS Code's own `terminalTaskSystem.ts`/`problemCollectors.ts` source (fetched from `microsoft/vscode` for this investigation) shows the clear-on-rerun logic (`StartStopProblemCollector`/`AbstractProblemCollector.done()`) is identical for every `RuntimeType`, with no `CustomExecution`-specific branch -- consistent with not finding a difference. The one apparent "confirmation" (`CustomExecution` stuck, `ProcessExecution` clearing) turned out to be a test-methodology artifact: editing the fixture via `fs.writeFileSync` instead of VS Code's document-edit/save APIs bypasses `onDidSaveTextDocument`, which starves the *separate* background live-diagnostics checker (`src/build/diagnostics.ts`, same "purebasic" owner) of the event it needs to refresh -- unrelated to the task system. With the fix properly isolated (task diagnostic only, via a file extension not registered as `purebasic` so the background checker never engages) and using the real compiler, `CustomExecution` cleared correctly on every one of 3 repeated runs; with both systems active and the raw-write methodology, *both* `CustomExecution` and `ProcessExecution` stayed stuck on every one of 4 repeated runs each. The `ProcessExecution` refactor was reverted (not worth the added `taskRunner.ts`/`ELECTRON_RUN_AS_NODE` complexity for an unproven bug) -- `src/build/taskProvider.ts` is back to `CustomExecution`. Before reopening this: reproduce with a real edit+save through VS Code's own APIs (not a raw disk write) and with the task diagnostic isolated from `diagnostics.ts` (e.g. an unregistered file extension), since every failure found so far traced back to that background system, not the task's own matcher.
  - **Separately, this investigation found and fixed a real, unrelated, confirmed bug in `src/build/diagnostics.ts`:** `check()` opens an included file via `vscode.workspace.openTextDocument()` purely to compute a diagnostic's range; that open fires `onDidOpenTextDocument`, which `extension.ts` forwards straight into `scheduleCheck()`, recursively scheduling a *second*, independent check that treats the include as its own main document and misattributes a duplicate diagnostic under a second ownership key that the M6 merge-by-owner logic then concatenates instead of dedupes. Fixed with a reference-counted `openingForDiagnostics` guard suppressing `scheduleCheck()` while `check()`'s own internal open is in flight for that URI. Confirmed live with the real `pbcompiler` and an `XIncludeFile`'d error (3 repeated runs, previously flaky/duplicating, now consistently correct).
